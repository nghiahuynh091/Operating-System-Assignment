 ...
 */
 ...
 pthread_mutex_t free_fp_lock = PTHREAD_MUTEX_INITIALIZER;
 ...
 {
    pthread_mutex_lock(&free_fp_lock);
    struct framephy_struct *fp = mp->free_fp_list;
 
    if (fp == NULL)
    {
       pthread_mutex_unlock(&free_fp_lock);
       return -1;
    }
    *retfpn = fp->fpn;
 ...
 {
    pthread_mutex_lock(&free_fp_lock);
     // Add validation to prevent segmentation fault
     if (mp == NULL || fpn < 0 || fpn >= mp->maxsz / PAGING_PAGESZ) {
       pthread_mutex_unlock(&free_fp_lock);
